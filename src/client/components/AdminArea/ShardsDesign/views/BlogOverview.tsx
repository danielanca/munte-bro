...
import { deleteProduct as deleteProductApi } from "../../../../services/emails";
...
const ProductsPage: React.FC = () => {
  ...
  const askDelete = (p: Product) =>
    setDeletePopup({ productInfo: String(p.title), productLink: p.ID, popUp: true });

  const dialogPopup = async (event: string, payload: string) => {
    if (event !== "modal_Event") return;
    if (payload === "YES" && delPopUp.productLink != null) {
      try {
        await deleteProductApi({ ID: delPopUp.productLink } as any);
    ...
    <Container fluid className="px-4">
      ...
      <Row className="mb-3">
        <Col lg={5} sm={12} xs={12} className="d-sm-flex d-md-block flex-sm-row gap-2">
          ...
          <Button size="sm" variant="secondary">Something else</Button>
        </Col>
      </Row>
